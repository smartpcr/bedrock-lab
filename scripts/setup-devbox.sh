...
if [ -f "/usr/bin/brew" ]; then
    ...
    echo "alias brew=\"$LINUXBREWHOME/bin/brew\"" >> ~/.zshrc
...
if [ -f "/usr/local/bin/terraform" ]; then
    ...
    # bedrock cannot use terraform above version 0.12.6
...
if [ -f "/usr/binfluxctl" ]; then
    ...
    # snapd not yet supported on WSL
